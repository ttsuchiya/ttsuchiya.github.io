...
const app = new Vue({
    ...
    methods: {
        initPlugin() {
            plugin.init('Data Moves Sequencer')
                .then(this.onGetData)
                ...
                if (notice.resource === 'documentChangeNotice') {
                    ...
                    if (notice.values.operation === 'selectCases') {
                        if (contextName === this.name) {
                            ...
                            if (notice.resource.includes(`[${this.focusedContext}]`) && this.recording) {
                    ...
                    } else if (['moveAttribute', 'createCollection', 'deleteCollection'].includes(notice.values.operation)) {
                        // TODO: Only query the changed context.
                        plugin.queryDataForContext(contextName).then(this.onGetData);

                        if (contextName !== this.name && this.recording && !this.replayingFromRecord) {
                            plugin.queryAttributeCollections(contextName).then(attrCollections => {
                                this.onAttributeMoved(contextName, attrCollections);
                            });
                        }
                    } else if (notice.values.operation === 'updateAttributes') {
                        // Look for the change of formula in attribute?

                        if (contextName !== this.name && this.recording && !this.replayingFromRecord && this.updateFormulaIncoming) {
                            this.onFormulaEdited(contextName, notice.values.result.attrs);
                            this.updateFormulaIncoming = false;
                        }
                    } else if (notice.values.operation === 'moveCases') {
                        if (contextName !== this.name && this.recording && !this.replayingFromRecord && this.incomingSortByAttr) {
                        ...
                        // TODO: Only query the changed context.
                        plugin.queryDataForContext(contextName).then(this.onGetData);
                    } else if (notice.values.operation === 'createCases') {
        ...
        setupUI() {
            ...
            let playbackToggle = new Nexus.Toggle('#playback-toggle', {
                size: [30, 15],
                state: this.playing
            });

            playbackToggle.on('change', v => {
                this.playing = v;
            });
        ...
        onGetData() {
            ...
            if (this.contexts) {
                ...
                filteredContexts.splice(filteredContexts.indexOf(this.name), 1);
